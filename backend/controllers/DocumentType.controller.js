...
exports.create = async (req, res) => {
  try {
    ...
    const detailExist = await DocumentType.findOne({ name });
    if (detailExist) {
    ...
    let newDetail = new DocumentType({
      name,
      slug: defaultSlug,
    });
...
exports.update = async (req, res) => {
  try {
    ...
    let updateDetail = await DocumentType.findById(updateId);
    if (!updateDetail)
...
exports.getAllData = async (req, res) => {
  try {
    ...
    let getData = await DocumentType.find({})
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .lean()
      .limit(parseInt(limit));

    const totalData = await DocumentType.countDocuments({});
...
exports.bulkDelete = async (req, res) => {
  try {
    ...
    const bulkDeleteAll = await DocumentType.find({ _id: { $in: delId } });
    ...
    await DocumentType.deleteMany({ _id: { $in: delId } });
