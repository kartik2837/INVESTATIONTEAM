...
exports.create = async (req, res) => {
  try {
    ...
    const detailExist = await JobCategory.findOne({ name });
    if (detailExist) {
...
exports.update = async (req, res) => {
  try {
    ...
    let updateDetail = await JobCategory.findById(updateId);
    if (!updateDetail)
...
exports.getAllData = async (req, res) => {
  try {
    ...
    let getData = await JobCategory.find({})
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .lean()
      .limit(parseInt(limit));

    const totalData = await JobCategory.countDocuments({});
...
exports.bulkDelete = async (req, res) => {
  try {
    ...
    const bulkDeleteAll = await JobCategory.find({ _id: { $in: delId } });
    ...
    await JobCategory.deleteMany({ _id: { $in: delId } });
