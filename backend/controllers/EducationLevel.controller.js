...
exports.create = async (req, res) => {
  try {
    ...
    const detailExist = await EducationLevel.findOne({ name });
    if (detailExist) {
    ...
    let newDetail = new EducationLevel({
      name,
      slug: defaultSlug,
    });
...
exports.update = async (req, res) => {
  try {
    ...
    let updateDetail = await EducationLevel.findById(updateId);
    if (!updateDetail)
...
exports.getAllData = async (req, res) => {
  try {
    ...
    let getData = await EducationLevel.find({})
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .lean()
      .limit(parseInt(limit));

    const totalData = await EducationLevel.countDocuments({});
...
exports.bulkDelete = async (req, res) => {
  try {
    ...
    const bulkDeleteAll = await EducationLevel.find({ _id: { $in: delId } });
    ...
    await EducationLevel.deleteMany({ _id: { $in: delId } });
