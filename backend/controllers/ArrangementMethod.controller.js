const ArrangementMethod = require("../models/ArrangementMethod.model");
...
exports.create = async (req, res) => {
  try {
    ...
    const detailExist = await ArrangementMethod.findOne({ name });
    if (detailExist) {
    ...
    let newDetail = new ArrangementMethod({
      name,
      slug: defaultSlug,
    });
...
exports.update = async (req, res) => {
  try {
    ...
    let updateDetail = await ArrangementMethod.findById(updateId);
    if (!updateDetail)
...
exports.getAllData = async (req, res) => {
  try {
    ...
    let getData = await ArrangementMethod.find({})
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .lean()
      .limit(parseInt(limit));

    const totalData = await ArrangementMethod.countDocuments({});
...
exports.bulkDelete = async (req, res) => {
  try {
    ...
    const bulkDeleteAll = await ArrangementMethod.find({ _id: { $in: delId } });
    ...
    await ArrangementMethod.deleteMany({ _id: { $in: delId } });
