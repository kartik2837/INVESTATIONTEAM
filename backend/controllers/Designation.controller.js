...
exports.create = async (req, res) => {
  try {
    ...
    const detailExist = await Designation.findOne({ name });
    if (detailExist) {
...
exports.update = async (req, res) => {
  try {
    ...
    let updateDetail = await Designation.findById(updateId);
    if (!updateDetail)
...
exports.getAllData = async (req, res) => {
  try {
    ...
    let getData = await Designation.find({})
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .lean()
      .limit(parseInt(limit));

    const totalData = await Designation.countDocuments({});
...
exports.bulkDelete = async (req, res) => {
  try {
    ...
    const bulkDeleteAll = await Designation.find({ _id: { $in: delId } });
    ...
    await Designation.deleteMany({ _id: { $in: delId } });
