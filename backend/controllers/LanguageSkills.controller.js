const LanguageSkills = require("../models/LanguageSkills.model");
...
exports.create = async (req, res) => {
  try {
    ...
    const detailExist = await LanguageSkills.findOne({ name });
    if (detailExist) {
    ...
    let newDetail = new LanguageSkills({
      name,
      slug: defaultSlug,
    });
...
exports.update = async (req, res) => {
  try {
    ...
    let updateDetail = await LanguageSkills.findById(updateId);
    if (!updateDetail)
...
exports.getAllData = async (req, res) => {
  try {
    ...
    let getData = await LanguageSkills.find({})
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .lean()
      .limit(parseInt(limit));

    const totalData = await LanguageSkills.countDocuments({});
...
exports.bulkDelete = async (req, res) => {
  try {
    ...
    const bulkDeleteAll = await LanguageSkills.find({ _id: { $in: delId } });
    ...
    await LanguageSkills.deleteMany({ _id: { $in: delId } });
