...
exports.create = async (req, res) => {
  try {
    ...
    const detailExist = await Skills.findOne({ name });
    if (detailExist) {
...
exports.update = async (req, res) => {
  try {
    ...
    let updateDetail = await Skills.findById(updateId);
    if (!updateDetail)
...
exports.getAllData = async (req, res) => {
  try {
    ...
    let getData = await Skills.find({})
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .lean()
      .limit(parseInt(limit));

    const totalData = await Skills.countDocuments({});
...
exports.bulkDelete = async (req, res) => {
  try {
    ...
    const bulkDeleteAll = await Skills.find({ _id: { $in: delId } });
    ...
    await Skills.deleteMany({ _id: { $in: delId } });
