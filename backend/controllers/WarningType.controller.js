const WarningType = require("../models/WarningType.model");
...
exports.create = async (req, res) => {
  try {
    ...
    const detailExist = await WarningType.findOne({ name });
    if (detailExist) {
...
exports.update = async (req, res) => {
  try {
    ...
    let updateDetail = await WarningType.findById(updateId);
    if (!updateDetail)
...
exports.getAllData = async (req, res) => {
  try {
    ...
    let getData = await WarningType.find({})
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .lean()
      .limit(parseInt(limit));

    const totalData = await WarningType.countDocuments({});
...
exports.bulkDelete = async (req, res) => {
  try {
    ...
    const bulkDeleteAll = await WarningType.find({ _id: { $in: delId } });
    ...
    await WarningType.deleteMany({ _id: { $in: delId } });
